...
class MeanLoss(nn.Module):
    ...
    def forward(self, outputs, labels, sen_groups, ad, a_map):
        ...
        for the_label in unique_labels:
            if (labels == the_label).sum() > 0:
                group_new = []
                for doms in a_map:
                    temp = [torch.eq(i,torch.Tensor(doms)) for i in torch.stack(ad, dim=1)]
                    ...
                    group_new.append(sen_groups[3][indices])
                    if(doms == [0,0,0,0]):
                        result = result + torch.FloatTensor([0.0]).contiguous().to(self.device)
                    elif(doms == [1,1,1,1]):
                        result = result + self.compute_mean_gap_group(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[1][labels_new == the_label], group_new[2][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [1,1,1,0]):
                        result = result + self.compute_mean_gap_triple(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[1][labels_new == the_label], group_new[2][labels_new == the_label])
                    elif(doms == [1,1,0,1]):
                        result = result + self.compute_mean_gap_triple(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[1][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [1,1,0,0]):
                        ...
                        result = result + self.compute_mean_gap_triple(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[2][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [1,0,1,0]):
                        result = result + self.compute_mean_gap_double(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[2][labels_new == the_label])
                    elif(doms == [1,0,0,1]):
                        result = result + self.compute_mean_gap_double(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [1,0,0,0]):
                        result = result + self.compute_mean_gap_single(outputs_new[labels_new == the_label], group_new[0][labels_new == the_label])
                    elif(doms == [0,1,1,1]):
                        result = result + self.compute_mean_gap_triple(outputs_new[labels_new == the_label], group_new[1][labels_new == the_label], group_new[2][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [0,1,1,0]):
                        result = result + self.compute_mean_gap_double(outputs_new[labels_new == the_label], group_new[1][labels_new == the_label], group_new[2][labels_new == the_label])
                    elif(doms == [0,1,0,1]):
                        result = result + self.compute_mean_gap_double(outputs_new[labels_new == the_label], group_new[1][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [0,1,0,0]):
                        result = result + self.compute_mean_gap_single(outputs_new[labels_new == the_label], group_new[1][labels_new == the_label])
                    elif(doms == [0,0,1,1]):
                        result = result + self.compute_mean_gap_double(outputs_new[labels_new == the_label], group_new[2][labels_new == the_label], group_new[3][labels_new == the_label])
                    elif(doms == [0,0,1,0]):
                        result = result + self.compute_mean_gap_single(outputs_new[labels_new == the_label], group_new[2][labels_new == the_label])
                    elif(doms == [0,0,0,1]):
                        result = result + self.compute_mean_gap_single(outputs_new[labels_new == the_label], group_new[3][labels_new == the_label])
                    else:
                        print(doms)
            else:
                print("Skipping regularization due to no samples")
